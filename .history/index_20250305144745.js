...
function getDiceRollMacro() {
    ...
    const rollReplace = (match, matchValue) => { // match param is required by replacement function signature
        console.debug('Dice: rollReplace called with match:', match, 'matchValue:', matchValue);
        ...
        try {
            ...
            console.debug('Dice: Got context:', context);
            ...
            console.debug('Dice: Sending system message:', rollMessage);
            context.sendSystemMessage('generic', rollMessage, { isSmallSys: true });
            console.debug('Dice: System message sent');
        } catch (error) {
            console.error('Dice: Error sending system message for roll', error);
        }
...
jQuery(function () {
    ...
    try {
        const context = getContext();
        console.debug('Dice: Registering macro, context:', context);
        if (context.registerMacro) {
            const macro = getDiceRollMacro();
            console.debug('Dice: Macro object:', macro);
            context.registerMacro(MODULE_NAME, macro);
            console.debug('Dice: Registered dice roll macro');
            
            // Test the macro
            const testText = "Testing {{roll: 2d6}} dice roll";
            console.debug('Dice: Testing macro with:', testText);
            const processed = testText.replace(macro.regex, macro.replace);
            console.debug('Dice: Processed result:', processed);
        } else {
