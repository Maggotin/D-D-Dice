...
jQuery(function () {
    ...
    try {
        const context = getContext();
        // console.debug('Dice: Registering macro, context available:', !!context);
        if (context.registerMacro) {
            const macro = getDiceRollMacro();
            // console.debug('Dice: Macro object created');
            ...
            $(document).on('click', '#send_but, #send_textarea', function() {
                // console.debug('Dice: Message send detected, checking for dice macros');
                const messageText = $('#send_textarea').val();
                if (messageText && messageText.includes('{{roll')) {
                    console.debug('Dice: Found potential dice roll in message:', messageText);
                }
            });
        } else {
