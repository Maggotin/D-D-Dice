...
function getDiceRollMacro() {
    ...
    const rollReplace = (match, matchValue) => {
        ...
        // Only add 1d prefix if it's a single number (representing sides of a die)
        // This allows advanced notation to pass through unchanged
        if (isDigitsOnly(formula) && !formula.includes('d')) {
            formula = `1d${formula}`;
        }
        ...
        // Just return the total - system messages will be handled by the extension
