...
import { eventSource, event_types, messageFormatting, saveChatDebounced, substituteParams } from '../../../../script.js';
...
/**
 * Process dice roll patterns in a message
...
const handleMessage = async(mesIdx) => {
    ...
    // Process {{roll::formula}} patterns
    const rollPattern = /{{roll[ : ]([^}]+)}}/gi;
    let match;
    while (match = text.match(rollPattern)) {
        isChanged = true;
        ...
        if (isDigitsOnly(formula)) {
            normalizedFormula = `1d${formula}`;
        }
        // Handle d20 case (no leading number)
        else if (formula.startsWith('d')) {
            normalizedFormula = '1' + formula;
        }

        if (!droll.validate(normalizedFormula)) {
            console.debug('Invalid dice formula:', formula);
            continue;
        }

        const result = droll.roll(normalizedFormula);
        if (result === false) {
            console.debug('Roll failed for formula:', formula);
            continue;
        }
        ...
        let rollMessage = '';
        const hasModifiers = /[kdr!<>]/i.test(normalizedFormula);
        
        if (hasModifiers) {
            rollMessage = `🎲 ${context.name1} rolls ${normalizedFormula}. Result: ${result.total} [${result.rolls}]`;

            if (result.keptDice && result.droppedDice?.length > 0) {
                rollMessage += `\n(Kept: [${result.keptDice.join(', ')}], Dropped: [${result.droppedDice.join(', ')}])`;
            }
            ...
            rollMessage = `🎲 ${context.name1} rolls ${normalizedFormula}. The result is: ${result.total} (${result.rolls})`;
        }

        // Replace the pattern with the roll result and message
        text = text.replace(match[0], rollMessage);
...
eventSource.on(event_types.USER_MESSAGE_RENDERED, (mesIdx)=>handleMessage(mesIdx));
eventSource.on(event_types.CHARACTER_MESSAGE_RENDERED, (mesIdx)=>handleMessage(mesIdx));
eventSource.on(event_types.MESSAGE_EDITED, (mesIdx)=>handleMessage(mesIdx));
