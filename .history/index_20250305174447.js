 ...
 * @param {boolean} asSystemMessage Whether to send as system message (default: true)
 * @returns {Promise<string>} Roll result
 */
async function doDiceRoll(customDiceFormula, quiet = false, asSystemMessage = true) {
            ...
            setTimeout(() => {
                try {
                    if (asSystemMessage) {
                        ...
                        context.sendMessage(rollMessage);
                    }
                } catch (error) {
                    console.error('Dice: Error sending message', error);
                }
            }, 10);
        }
        return String(result.total);
...
jQuery(function () {
        ...
        if (registerMacro) {
            registerMacro(MODULE_NAME, {
                ...
                replace: async (match, formula) => {
                    ...
                    const result = await doDiceRoll(formula, false, false); // Send as regular message
                    return result || match;
                }
            });
        }
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        callback: (args, value) => {
            ...
            const asSystemMessage = quiet; // If quiet is false, send as user message
            return doDiceRoll(String(value), quiet, asSystemMessage);
        },
        ...
        namedArgumentList: [
            SlashCommandNamedArgument.fromProps({
                name: 'quiet',
                description: 'Do not display the result in chat (default: true)',
                isRequired: false,
