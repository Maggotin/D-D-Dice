...
async function doDiceRoll(customDiceFormula, quiet = true, sendas = 'system') {
    ...
    if (isValid) {
        ...
        if (!quiet) {
            ...
            const message = `${context.name1} rolls ${value}. The result is: ${result.total} (${result.rolls})`;
            ...
            switch(sendas?.toLowerCase()) {
                case 'system':
                    ...
                    break;
                case 'user':
                    context.sendMessageAs(context.name1, message);
                    break;
                case 'char':
                    context.sendMessageAs(context.name2, message);
                    break;
            }
        }
        return String(result.total);
...
jQuery(function () {
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        namedArgumentList: [
            ...
            SlashCommandNamedArgument.fromProps({
                name: 'sendas',
                description: 'How to send the message: "system" (small system message), "user" (as user), "char" (as character)',
                isRequired: false,
