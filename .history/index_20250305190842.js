...
const droll = (() => {
    ...
    function tokenize(formula) {
        ...
        console.log(`Tokenizing formula: "${formula}"`);
        
        // Handle digit-only case
        if (isDigitsOnly(formula)) {
            return {
                count: 1,
                sides: parseInt(formula),
                keepDrop: null,
                keepDropCount: null,
                reroll: null,
                rerollOp: null,
        ...
        if (!match) {
            console.error(`No regex match for formula: "${formula}"`);
            return null;
        }
    ...
    function validate(formula) {
        // Handle single number (sides) case
        if (isDigitsOnly(formula)) return true;
        
        // Trim and normalize the formula
        formula = formula.trim();
        
        // Handle '1+2' and other basic arithmetic expressions
        if (/^\d+[+\-]\d+$/.test(formula)) return true;
        
        // Standard validation with regex
        console.log(`Validating formula: "${formula}"`);
    ...
    function roll(formula) {
        ...
        if (!token) {
            console.error(`Failed to tokenize formula: "${formula}"`);
            return false;
        }
