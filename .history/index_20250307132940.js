...
class DiceState {
    ...
    loadSettings() {
        const savedSettings = localStorage.getItem('diceExtensionSettings');
        if (savedSettings) {
            try {
                const parsed = JSON.parse(savedSettings);
                this.settings = { ...this.settings, ...parsed };
            } catch (error) {
                console.error('Failed to load dice extension settings:', error);
            }
        }
    }
    ...
    saveSettings() {
        try {
            localStorage.setItem('diceExtensionSettings', JSON.stringify(this.settings));
        } catch (error) {
            console.error('Failed to save dice extension settings:', error);
        }
    }

    addToHistory(roll) {
        if (!this.settings.keepRollHistory) return;
        
        this.rollHistory.unshift({
            ...roll,
            timestamp: Date.now(),
            character: getContext().name2
        });
...
// Development logging
function logDevelopment(type, message, data = null) {
    const logEntry = {
        type,
        message,
        data,
    ...
    console.log(`[D&D Dice] ${type}: ${message}`, data || '');
}
 ...
 */
async function doDiceRoll(customDiceFormula, quiet = state.settings.defaultQuiet) {
    ...
    logDevelopment('ROLL_ATTEMPT', `Attempting to roll: ${value}`, { isValid });
    ...
    if (isValid) {
        ...
        logDevelopment('ROLL_SUCCESS', `Roll successful: ${value}`, result);
        return String(result.total);
    } else {
        toastr.warning('Invalid dice formula');
        logDevelopment('ROLL_ERROR', `Invalid formula: ${value}`);
        return '';
    }
}
...
function handleMessageReceived(data) {
    // Check for dice roll commands in messages
    if (data.message && data.message.includes('/roll')) {
        logDevelopment('MESSAGE_RECEIVED', 'Dice roll command detected in message');
    }
}

function handleChatChanged() {
    // Reset state or update UI as needed when chat changes
    logDevelopment('CHAT_CHANGED', 'Chat context changed');
}
...
jQuery(async function () {
    try {
        logDevelopment('INIT', 'Initializing D&D Dice extension');
        
        await loadDependencies();
        logDevelopment('DEPENDENCIES', 'Dependencies loaded successfully');
        ...
        logDevelopment('INIT_COMPLETE', 'D&D Dice extension initialized successfully');
    } catch (error) {
        console.error('Failed to load D&D Dice extension:', error);
        logDevelopment('INIT_ERROR', 'Failed to initialize extension', error);
    }
});
