...
const droll = (() => {
    ...
    function roll(formula) {
        ...
        try {
            ...
            // Get the detailed output from the roll
            const detailedOutput = diceRoll.output.replace(`${formula}: `, '');
            
            // Split the output into total and rolls
            const match = detailedOutput.match(/^(\d+)\s*\((.*)\)$/);
            
            if (!match) {
                // Handle cases where there might not be individual rolls (like static numbers)
                return {
            ...
            return {
                total: diceRoll.total,
                rolls: match[2], // This contains the detailed roll results
                formula: formula
            };
        } catch (error) {
...
async function doDiceRoll(customDiceFormula, quiet = false) {
    ...
    if (isValid) {
        ...
        // For macros, we need to return just the total as a string
