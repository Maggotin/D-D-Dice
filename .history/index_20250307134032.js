...
const droll = (() => {
    ...
    function roll(formula) {
        ...
        try {
            ...
            // Extract the individual dice rolls and modifiers
            const rollsData = diceRoll.notation.dice.map(die => {
                // Get the individual rolls
                const rolls = die.rolls.map(r => {
                    let rollStr = String(r.value);
                    // Add roll modifiers if present (like dropped, exploded, etc.)
                    if (r.modifiers && r.modifiers.length > 0) {
                        rollStr += `[${r.modifiers.join(',')}]`;
                    }
                    return rollStr;
                });
                
                // Join the rolls with commas
                return rolls.join(', ');
            }).join('; ');
 ...
 * @returns {Promise<Object>} Roll result object containing total and rolls
 */
    ...
    try {
        ...
        registerFunctionTool({
            ...
            action: async (args) => {
                ...
                const rollResult = await doDiceRoll(args.formula, true);
                const result = args.who ? 
                    `${args.who} rolls a ${args.formula}. The result is: ${rollResult.total} (${rollResult.rolls})` : 
                    `The result of a ${args.formula} roll is: ${rollResult.total} (${rollResult.rolls})`;
                return result;
            },
            formatMessage: () => '',
        });
