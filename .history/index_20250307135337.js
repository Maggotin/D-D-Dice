...
const droll = (() => {
    ...
    function roll(formula) {
        ...
        try {
            ...
            // Get the raw rolls array for detailed output
            const rolls = diceRoll.rolls.flat().map(r => {
                if (typeof r === 'object') {
...
async function doDiceRoll(customDiceFormula, quiet = false) {
    ...
    if (isValid) {
        ...
        // For macros, return both the total and individual rolls
    ...
    try {
        ...
        registerMacro({
            ...
            handler: async (args) => {
                ...
                return result; // Now returns both total and individual rolls
            },
        });
    } catch (error) {
