    ...
    function roll(formula) {
        ...
        try {
            ...
            diceRoll.dice.forEach(die => {
                // Get the rolls for this die
                const dieRolls = die.rolls.map(roll => {
                    let rollText = String(roll.value);
                    
                    // Add any modifiers (like dropped, exploded, etc)
                    if (roll.modifiers?.length > 0) {
                        rollText += `[${roll.modifiers.join(',')}]`;
                    }
                    
                    // Add any flags
                    if (roll.dropped) rollText += '[dropped]';
                    if (roll.exploded) rollText += '[exploded]';
                    if (roll.rerolled) rollText += '[rerolled]';
                    
                    return rollText;
                });
                
                rollDetails.push(dieRolls);
            });
            
            // Flatten and join all roll details
            const rolls = rollDetails.flat().join(', ');
