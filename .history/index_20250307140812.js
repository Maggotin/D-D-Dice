...
const droll = (() => {
    ...
    function roll(formula) {
        ...
        try {
            ...
            // Extract detailed roll information using the output property
            const rollDetails = [];
            
            // Process each dice roll output
            diceRoll.output.forEach(output => {
                if (typeof output === 'object' && output.rolls) {
                    output.rolls.forEach(roll => {
                        if (Array.isArray(roll)) {
                            const rollText = roll.map(r => {
                                let text = r.value.toString();
                                if (r.modifiers && r.modifiers.length > 0) {
                                    text += `[${r.modifiers.join(',')}]`;
                                }
                                return text;
                            }).join(', ');
                            rollDetails.push(rollText);
                        } else if (typeof roll === 'number') {
                            rollDetails.push(roll.toString());
                        }
                    });
                }
            });
