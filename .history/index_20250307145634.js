        ...
        registerMacro('roll', {
            ...
            handler: async (args) => {
                ...
                try {
                    ...
                    if (isDigitsOnly(formula)) {
                        const diceRoll = new rpgDiceRoller.DiceRoll(`1d${formula}`);
                        return `${diceRoll.total} (rolled as 1d${formula}: ${diceRoll.notation})`;
                    }
                    
                    // Handle 'd' prefix case (e.g., {{roll:d20}} -> {{roll:1d20}})
                    if (formula.startsWith('d') && isDigitsOnly(formula.substring(1))) {
                        const diceRoll = new rpgDiceRoller.DiceRoll(`1${formula}`);
                        return `${diceRoll.total} (rolled as 1${formula}: ${diceRoll.notation})`;
                    }
                    ...
                    return `[Error: Invalid formula "${args.trim()}". Use format like 2d6, 4d6kh3, or 2d20!]`;
                }
            },
        });
    } catch (error) {
