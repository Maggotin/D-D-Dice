...
function registerMacros() {
    try {
        ...
        // Register the roll macro with enhanced functionality
        registerMacro('roll', {
            name: 'roll',
            ...
            handler: async (args) => {
                ...
                try {
                    // Handle digit-only case (e.g., {{roll:20}} -> {{roll:1d20}})
                    if (isDigitsOnly(input)) {
                        const diceRoll = new rpgDiceRoller.DiceRoll(`1d${input}`);
                        return `${diceRoll.total} (rolled as 1d${input}: ${diceRoll.notation})`;
                    }
                    
                    // Handle 'd' prefix case (e.g., {{roll:d20}} -> {{roll:1d20}})
                    if (input.startsWith('d') && isDigitsOnly(input.substring(1))) {
                        const diceRoll = new rpgDiceRoller.DiceRoll(`1${input}`);
                        return `${diceRoll.total} (rolled as 1${input}: ${diceRoll.notation})`;
                    }

                    // Create roll with advanced notation support
                    const diceRoll = new rpgDiceRoller.DiceRoll(input);
                    
                    // Format detailed roll results
                    let rollDetails = [];
                    diceRoll.rolls.forEach(rollGroup => {
                        if (Array.isArray(rollGroup)) {
                            const groupDetails = rollGroup.map(result => {
                    ...
                    return `${diceRoll.total} (${rollDetails.join(' ')})`;
                } catch (error) {
