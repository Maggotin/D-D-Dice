...
function registerMacros() {
    try {
        ...
        // Register both roll and rolls macros for compatibility
        const macroHandler = async (args) => {
            // Ensure we have a string and handle empty/undefined cases
            ...
            try {
                ...
                // Handle digit-only case (e.g., {{rolls:20}} -> {{rolls:1d20}})
                if (isDigitsOnly(cleanInput)) {
                    const formula = `1d${cleanInput}`;
                    const roll = await doDiceRoll(formula, true);
                    return roll || `[Error: Failed to roll ${formula}]`;
                }
                
                // Handle 'd' prefix case (e.g., {{rolls:d20}} -> {{rolls:1d20}})
                if (cleanInput.startsWith('d') && isDigitsOnly(cleanInput.substring(1))) {
                    const formula = `1${cleanInput}`;
                    const roll = await doDiceRoll(formula, true);
                    return roll || `[Error: Failed to roll ${formula}]`;
                }

                // Handle all other cases using doDiceRoll
                const roll = await doDiceRoll(cleanInput, true);
                return roll || `[Error: Failed to roll ${cleanInput}]`;
                
            } catch (error) {
                console.error('Macro roll failed:', error);
                return `[Error: Invalid formula "${input}". Use format like 2d6, 4d6kh3, or 2d20!]`;
            }
        };
        ...
        // Register the rolls macro (alternative name)
        registerMacro('rolls', {
            name: 'rolls',
