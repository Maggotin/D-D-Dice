 ...
 */
function getDiceRollMacros() {
    // Support both {{roll:formula}} and {{rolls:formula}} patterns
    const rollPattern = /{{(?:roll|rolls)[ : ]([^}]+)}}/gi;
    const rollReplace = (match, matchValue) => {
        ...
        if (result === false) return '[Roll failed]';
        
        // Return both total and individual rolls
...
function registerMacros() {
    try {
        // Get the macro registration function from SillyTavern's context
        const context = getContext();
        if (!context) {
            console.debug('Context not available');
            return;
        }

        // Register the dice roll macro with SillyTavern's macro system
        if (typeof context.addMacro === 'function') {
            context.addMacro(getDiceRollMacros());
        } else {
            console.debug('Macro registration not supported');
        }
    } catch (error) {
