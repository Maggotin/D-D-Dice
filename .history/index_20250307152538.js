    ...
    try {
        ...
        registerMacro('roll', {
            ...
            handler: async (args) => {
                ...
                // Use substituteParams to process the input as if it was a macro
                const processedInput = substituteParams(`{{roll:${input}}}`);
                
                if (processedInput === `{{roll:${input}}}`) {
                    // If the input wasn't processed, it means it wasn't a valid roll
                    try {
                        // Try direct rolling as fallback
                        if (isDigitsOnly(input)) {
                            const result = droll.roll(`1d${input}`);
                            return result ? `${result.total} (${result.rolls})` : '[Roll failed]';
                        }
                        
                        if (input.startsWith('d') && isDigitsOnly(input.substring(1))) {
                            const result = droll.roll(`1${input}`);
                            return result ? `${result.total} (${result.rolls})` : '[Roll failed]';
                        }

                        const result = droll.roll(input);
                        return result ? `${result.total} (${result.rolls})` : '[Roll failed]';
                    } catch (error) {
        ...
        registerMacro('rolls', {
            ...
            handler: async (args) => {
                const input = String(args ?? '').trim();
                // Use substituteParams to process the input as if it was a macro
                const processedInput = substituteParams(`{{rolls:${input}}}`);
                
                if (processedInput === `{{rolls:${input}}}`) {
                    // If the input wasn't processed, it means it wasn't a valid roll
                    try {
                        // Try direct rolling as fallback
                        if (isDigitsOnly(input)) {
