...
function registerMacros() {
    try {
        ...
        // Create a single DiceRoller instance for all rolls
        const roller = new rpgDiceRoller.DiceRoller();

        const handleDiceRoll = (input) => {
            ...
            try {
                // Handle single number case (e.g., "20" -> "1d20")
                if (isDigitsOnly(input)) {
                    input = `1d${input}`;
                }
                
                // Handle shorthand notation (e.g., "d20" -> "1d20")
                if (input.startsWith('d') && isDigitsOnly(input.substring(1))) {
                    input = `1${input}`;
                }

                // Parse the notation first to validate it
                const notation = rpgDiceRoller.DiceRoll.parseNotation(input);
                if (!notation || notation.length === 0) {
                    return `[Error: Invalid dice notation "${input}"]`;
                }

                // Roll the dice
                const roll = roller.roll(input);
                
                // Format the output using the built-in methods
                const total = roll.total;
                const notation_string = roll.notation;
                const output = roll.output.replace(/\[[^\]]+\]/g, ''); // Remove the roll type indicators
                
                return `${total} (${output})`;
            } catch (error) {
                console.error('Roll failed:', error);
                if (error instanceof rpgDiceRoller.NotationError) {
                    return `[Error: Invalid dice notation "${input}". Use format like 2d6, 4d6kh3, or 2d20!]`;
                } else if (error instanceof rpgDiceRoller.DiceRollError) {
                    return `[Error: Failed to roll "${input}". ${error.message}]`;
                }
                return `[Error: Failed to process "${input}"]`;
            }
        };

        // Register both roll and rolls patterns with the same handler
        ['roll', 'rolls'].forEach(name => {
            registerMacro(name, {
                name,
                description: `Roll dice using various formats (e.g., {{${name}:2d6}}, {{${name}:4d6kh3}}, {{${name}:2d20!}}, {{${name}:1d20+5}})`,
                handler: async (args) => handleDiceRoll(args),
            });
        });
    } catch (error) {
