        ...
        ['roll', 'rolls'].forEach(name => {
            registerMacro(name, {
                ...
                handler: async (args) => {
                    ...
                    // First try to process any nested macros
                    const processedInput = substituteParams(`{{${name}:${input}}}`);
                    if (processedInput !== `{{${name}:${input}}}`) {
                        return processedInput;
                    }

                    // If no nested macros, handle the dice roll
                    return handleDiceRoll(input);
                },
            });
        });
    } catch (error) {
