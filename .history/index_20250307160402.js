...
/**
 * Format a dice roll message consistently
 * @param {string} formula The dice formula used
 * @param {string} resultString The roll result
 * @param {string} [roller] Optional name of who rolled
 * @param {boolean} [isSystem] Whether this is a system message
 * @returns {string} Formatted message
 */
function formatDiceMessage(formula, resultString, roller = null, isSystem = false) {
    const message = roller 
        ? `${roller} rolls ${formula}. Result: ${resultString}`
        : `Rolling ${formula}. Result: ${resultString}`;
 ...
 */
async function doDiceRoll(customDiceFormula, quiet = false, roller = null) {
    ...
    if (isValid) {
        ...
        if (!quiet) {
            ...
            const message = formatDiceMessage(value, resultString, roller || context.name1, true);
            context.sendSystemMessage('generic', message, { isUser: true });
        }
    ...
    try {
        ...
        registerFunctionTool({
            ...
            action: async (args) => {
                ...
                const roll = await doDiceRoll(args.formula, true, args.who);
                return formatDiceMessage(args.formula, roll, args.who);
            },
            formatMessage: () => '',
        });
    } catch (error) {
    ...
    try {
        ...
        SlashCommandParser.addCommandObject(SlashCommand.fromProps({
            ...
            callback: (args, value) => {
                ...
                const context = getContext();
                return doDiceRoll(String(value), quiet, context.name1);
            },
            helpString: 'Roll the dice using various formats (e.g., 2d6, 4d6kh3, 2d20!, 1d20+5).',
            returns: 'roll result with format: "Name rolls formula. Result: X (details)"',
            namedArgumentList: [
