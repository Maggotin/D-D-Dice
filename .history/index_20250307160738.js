...
async function doDiceRoll(customDiceFormula, quiet = false) {
    ...
    if (isValid) {
        ...
        // Format the result with styling
        const resultString = `**${result.total}** _(${result.rolls})_`;
        
        if (!quiet) {
            const context = getContext();
            context.sendSystemMessage('generic', `${context.name1} rolls \`${value}\`. Result: ${resultString}`, { 
                isUser: true,
                isSystem: false,
                sanitizerOverrides: {
        ...
        registerFunctionTool({
            ...
            action: async (args) => {
                ...
                const result = args.who ? 
                    `${args.who} rolls \`${args.formula}\`. Result: ${roll}` : 
                    `Result of \`${args.formula}\`: ${roll}`;
                return result;
            },
            formatMessage: () => '',
        });
