 ...
 * @param {Object} options Additional options for output formatting
 * @param {boolean} options.isSmallSys Display as small system message
 * @param {boolean} options.forceAvatar Force avatar display
 * @param {boolean} options.isUser Mark as user message
 * @returns {Promise<string>} Roll result
 */
async function doDiceRoll(customDiceFormula, quiet = false, options = {}) {
    ...
    if (isValid) {
        ...
        if (!quiet) {
            ...
            const messageOptions = {
                isSmallSys: options.isSmallSys ?? true,
                forceAvatar: options.forceAvatar ?? false,
                isUser: options.isUser ?? true
            };
            context.sendSystemMessage('generic', `${context.name1} rolls a ${value}. The result is: ${resultString}`, messageOptions);
        }
    ...
    try {
        ...
        registerFunctionTool({
            ...
            action: async (args) => {
                ...
                const roll = await doDiceRoll(args.formula, true, { isSmallSys: true, forceAvatar: false });
...
function registerMacros() {
    try {
        ...
        // Register the roll macro using pattern matching
        const rollPattern = /{{roll[s]?[ : ]([^}]+)}}/gi;
        ...
        const rollReplace = (match, matchValue) => {
            ...
            try {
                ...
                return `[Error: Invalid formula "${formula}". Use format like 2d6, 4d6kh3, or 2d20!]`;
            }
        };

        // Register both roll and rolls patterns
        registerMacro('roll', {
            name: 'roll',
            description: 'Roll dice using various formats (e.g., {{roll:2d6}}, {{roll:4d6kh3}}, {{roll:2d20!}}, {{roll:1d20+5}})',
            handler: async (args) => {
                const input = String(args ?? '').trim();
                const match = rollPattern.exec(`{{roll:${input}}}`);
                if (match) {
                    return rollReplace(match[0], match[1]);
                }
                return `[Error: Invalid formula "${input}"]`;
            },
        });
        ...
        registerMacro('rolls', {
            name: 'rolls',
            description: 'Roll dice using various formats (e.g., {{rolls:2d6}}, {{rolls:4d6kh3}}, {{rolls:2d20!}}, {{rolls:1d20+5}})',
            handler: async (args) => {
                const input = String(args ?? '').trim();
                const match = rollPattern.exec(`{{rolls:${input}}}`);
                if (match) {
                    return rollReplace(match[0], match[1]);
                }
                return `[Error: Invalid formula "${input}"]`;
    ...
    try {
        ...
        SlashCommandParser.addCommandObject(SlashCommand.fromProps({
            ...
            callback: (args, value) => {
                ...
                const options = {
                    isSmallSys: isTrueBoolean(String(args.isSmallSys)),
                    forceAvatar: isTrueBoolean(String(args.forceAvatar)),
                    isUser: isTrueBoolean(String(args.isUser))
                };
                return doDiceRoll(String(value), quiet, options);
            },
            ...
            namedArgumentList: [
                ...
                SlashCommandNamedArgument.fromProps({
                    name: 'isUser',
                    description: 'Send as user message',
                    isRequired: false,
                    typeList: [ARGUMENT_TYPE.BOOLEAN],
                ...
                SlashCommandNamedArgument.fromProps({
                    name: 'isSmallSys',
                    description: 'Display as small system message',
                    isRequired: false,
                    typeList: [ARGUMENT_TYPE.BOOLEAN],
                    defaultValue: String(true),
                    enumProvider: commonEnumProviders.boolean('trueFalse'),
                ...
                SlashCommandNamedArgument.fromProps({
                    name: 'forceAvatar',
                    description: 'Force avatar display',
                    isRequired: false,
                    typeList: [ARGUMENT_TYPE.BOOLEAN],
                    defaultValue: String(false),
                    enumProvider: commonEnumProviders.boolean('trueFalse'),
