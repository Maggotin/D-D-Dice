...
function registerMacros() {
    try {
        ...
        function getDiceRollMacro() {
            // Support both roll and rolls patterns
            ...
            const rollReplace = (match, matchValue) => {
                ...
                try {
                    ...
                    // Return detailed roll result including individual dice
        ...
        registerMacro('roll', {
            ...
            handler: async (args) => {
                ...
                const match = diceRollMacro.regex.exec(`{{roll:${input}}}`);
                if (match) {
        ...
        registerMacro('rolls', {
            ...
            handler: async (args) => {
                const input = String(args ?? '').trim();
                const match = diceRollMacro.regex.exec(`{{rolls:${input}}}`);
                if (match) {
                    return diceRollMacro.replace(match[0], match[1]);
                }
                return `[Error: Invalid formula "${input}"]`;
