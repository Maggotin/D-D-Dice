...
function registerMacros() {
    try {
        ...
        function getDiceRollMacro() {
            // Support both roll and rolls patterns but without global flag
            ...
            const rollReplace = (match, matchValue) => {
                ...
                try {
                    ...
                    // Mark the result as processed and return detailed roll result
                    return `[Rolled] ${result.total} (${result.rolls})`;
                } catch (error) {
        ...
        registerMacro('roll', {
            ...
            handler: async (args) => {
                ...
                const match = input.match(diceRollMacro.regex);
                if (match) {
                    return diceRollMacro.replace(match[0], match[1]);
                }
                return `[Error: Invalid formula "${input}"]`;
            },
        });

        // Also register the rolls variant with the same non-recursive behavior
        registerMacro('rolls', {
            name: 'rolls',
