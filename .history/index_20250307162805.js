...
async function doDiceRoll(customDiceFormula, quiet = false) {
    ...
    if (isValid) {
        ...
        if (!quiet) {
            ...
            context.sendSystemMessage('generic', `${context.name1} rolls a ${value}. The result is: ${resultString}`, { isUser: true });
        }
...
function registerMacros() {
    try {
        ...
        /**
         * @returns {Object} The dice roll macro
         */
        function getDiceRollMacro() {
            // Support both roll and rolls patterns, handle entire input string
            const rollPattern = /{{roll[s]?[ :]([^}]+)}}/i;
            
            const rollReplace = (match, matchValue) => {
                // Prevent re-processing already processed rolls
                if (match.includes('[Rolled]')) {
                    return match;
                }
                ...
                try {
                    ...
                    // Return result without [Rolled] marker for cleaner output
                    return `${result.total} (${result.rolls})`;
                } catch (error) {
                    console.error('Roll failed:', error);
                    return `[Error: Invalid formula "${formula}". Use format like 2d6, 4d6kh3, 2d20!, or 1d20+5]`;
                }
            };
            ...
            return {
                ...
                processMessage: (message) => {
                    ...
                    // Process all roll macros in the message
                    while ((match = rollPattern.exec(processed)) !== null) {
                        const replacement = rollReplace(match[0], match[1]);
        ...
        const diceRollMacro = getDiceRollMacro();
        ...
        registerMacro('roll', {
            ...
            handler: async (args) => {
                ...
                // Handle both direct formulas and full macro syntax
                const macroText = input.includes('{{') ? input : `{{roll:${input}}}`;
                return diceRollMacro.processMessage(macroText);
            },
        });
        ...
        registerMacro('rolls', {
            ...
            handler: async (args) => {
                const input = String(args ?? '').trim();
                const macroText = input.includes('{{') ? input : `{{rolls:${input}}}`;
                return diceRollMacro.processMessage(macroText);
            },
        });
    } catch (error) {
