...
function registerMacros() {
    try {
        ...
        SillyTavern.getContext().registerMacro('rolls', () => {
            ...
            const rollReplace = async (match, matchValue) => {
                ...
                console.debug(`Dice roll result: ${roll}`);
                if (!roll) return '';
                return roll.split(' ')[0]; // Get just the number result
            };

            return { regex: rollPattern, replace: rollReplace };
        });
        console.debug('Dice roll macro registered successfully');
