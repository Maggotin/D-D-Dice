...
async function doDiceRoll(customDiceFormula, quiet = false) {
    ...
    const isValid = diceroll.validate(value);

    if (isValid) {
        const result = diceroll.roll(value);
        if (!result) {
            return '[Roll failed]';
        }
...
function registerMacros() {
    try {
        console.debug('Registering dice roll macro...');
        
        // Register the macro with the exact name that matches what's in the message
        SillyTavern.getContext().registerMacro('roll', () => {
            console.debug('Dice roll macro pattern setup');
            const rollPattern = /{{roll:([^}]+)}}/gi;
            const rollReplace = (match, matchValue) => {
                console.debug(`Dice roll macro triggered with: ${matchValue}`);
                ...
                SillyTavern.getContext().substituteParamsExtended(droll)
                17: 29: 14.442 macros.js: 562 Macro content can't be replaced: /<USER>/gi in [object Object] TypeError: content.replace is not a function
    at evaluateMacros(macros.js: 560: 31)
    at substituteParams(script.js: 2735: 12)
    at Object.substituteParamsExtended(script.js: 2658: 12)
                at < anonymous >: 1: 26
evaluateMacros @macros.js: 562
substituteParams @script.js: 2735
substituteParamsExtended @script.js: 2658
                    (anonymous) @VM21158: 1
                17: 29: 14.442 macros.js: 562 Macro content can't be replaced: /<BOT>/gi in [object Object] TypeError: content.replace is not a function
    at evaluateMacros(macros.js: 560: 31)
    at substituteParams(script.js: 2735: 12)
    at Object.substituteParamsExtended(script.js: 2658: 12)
                at < anonymous >: 1: 26
...
substituteParamsExtended @script.js: 2658
                    (anonymous) @VM21158: 1
                17: 29: 14.442 macros.js: 562 Macro content can't be replaced: /<CHAR>/gi in [object Object] TypeError: content.replace is not a function
    at evaluateMacros(macros.js: 560: 31)
    at substituteParams(script.js: 2735: 12)
    at Object.substituteParamsExtended(script.js: 2658: 12)
                at < anonymous >: 1: 26
...
substituteParamsExtended @script.js: 2658
                    (anonymous) @VM21158: 1
                17: 29: 14.442 macros.js: 562 Macro content can't be replaced: /<CHARIFNOTGROUP>/gi in [object Object] TypeError: content.replace is not a function
    at evaluateMacros(macros.js: 560: 31)
    at substituteParams(script.js: 2735: 12)
    at Object.substituteParamsExtended(script.js: 2658: 12)
                at < anonymous >: 1: 26
...
substituteParamsExtended @script.js: 2658
                    (anonymous) @VM21158: 1
                17: 29: 14.442 macros.js: 562 Macro content can't be replaced: /<GROUP>/gi in [object Object] TypeError: content.replace is not a function
    at evaluateMacros(macros.js: 560: 31)
    at substituteParams(script.js: 2735: 12)
    at Object.substituteParamsExtended(script.js: 2658: 12)
                at < anonymous >: 1: 26
...
substituteParamsExtended @script.js: 2658
                    (anonymous) @VM21158: 1
                17: 29: 14.471 macros.js: 555 Uncaught TypeError: content.includes is not a function
                    at evaluateMacros(macros.js: 555: 61)
    at substituteParams(script.js: 2735: 12)
    at Object.substituteParamsExtended(script.js: 2658: 12)
                at < anonymous >: 1: 26
evaluateMacros @macros.js: 555
substituteParams @script.js: 2735
substituteParamsExtended @script.js: 2658
                    (anonymous) @VM21158: 1
                try {
                    const result = diceroll.roll(formula);
                    console.debug(`Dice roll result:`, result);
                    if (!result) return '';
                    return String(result.total);
                } catch (error) {
                    console.debug(`Dice roll error:`, error);
                    return '';
                }
            };
        ...
        // Also register the 'rolls' variant
        SillyTavern.getContext().registerMacro('rolls', () => {
            console.debug('Dice rolls macro pattern setup');
            const rollPattern = /{{rolls:([^}]+)}}/gi;
            const rollReplace = (match, matchValue) => {
                console.debug(`Dice rolls macro triggered with: ${matchValue}`);
                let formula = matchValue.trim();
        ...
        console.debug('Dice roll macros registered successfully');
