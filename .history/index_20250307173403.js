...
function registerMacros() {
    try {
        ...
        // Create a custom macro object for dice rolls
        const diceRollMacros = {
            roll: (formula) => {
                console.debug(`Dice roll macro triggered with:`, formula);
        ...
        // Hook into the message processing pipeline
        const originalSubstituteParams = context.substituteParamsExtended;
        
        context.substituteParamsExtended = function(content, additionalMacro = {}, postProcessFn = (x) => x) {
            // Add our dice roll macros to the additional macros
            const combinedMacros = { ...additionalMacro, ...diceRollMacros };
            
            // Call the original function with our extended macros
            return originalSubstituteParams(content, combinedMacros, postProcessFn);
        };
        
        console.debug('Dice roll macros registered successfully via substituteParamsExtended');
    } catch (error) {
