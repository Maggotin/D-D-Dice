...
function registerMacros() {
    try {
        console.debug('Registering dice roll macros...');
        
        // Add a global event listener for message processing
        jQuery(document).on('messageRendered', function(event, messageElement) {
            const $message = jQuery(messageElement);
            const originalText = $message.html();
            
            // Skip if no dice macros
            if (!originalText.includes('{{roll') && !originalText.includes('{{rolls')) {
                return;
            }
            
            console.debug('Found potential dice roll in message:', originalText);
            
            // Process roll macros
            const processedText = SillyTavern.getContext().substituteParamsExtended(originalText, {
                roll: (formula) => {
                    console.debug(`Processing roll with formula: ${formula}`);
                    if (!formula) return '';
                    
                    let diceFormula = formula.trim();
                    
                    // Handle digit-only case (e.g., "20" -> "1d20")
                    if (isDigitsOnly(diceFormula)) {
                        diceFormula = `1d${diceFormula}`;
                    }
                    
                    // Handle 'd' prefix case (e.g., "d20" -> "1d20")
                    if (diceFormula.startsWith('d') && isDigitsOnly(diceFormula.substring(1))) {
                        diceFormula = `1${diceFormula}`;
                    }
                    
                    try {
                        // Use the diceroll object from the outer scope
                        if (!diceroll.validate(diceFormula)) {
                            console.debug(`Invalid dice formula: ${diceFormula}`);
                            return '';
                        }
                        
                        const result = diceroll.roll(diceFormula);
                        console.debug(`Dice roll result for ${diceFormula}:`, result);
                        if (!result) return '';
                        return String(result.total);
                    } catch (error) {
                        console.debug(`Error rolling dice: ${error.message}`);
                        return '';
                    }
                },
                
                // Alias for roll
                rolls: function(formula) {
            ...
            if (processedText !== originalText) {
                console.debug('Updating message with processed dice rolls');
                $message.html(processedText);
            }
        });
